...
- `密码前缀`：**可选**。填了就是姓名缩写，不填也能开户 —— 密码是「前缀 + 后缀 + 工号后6位」，
  前缀只是可选段。程序不做拼音转换（避免引入依赖和多音字出错）
...
**密码从哪来**：批量建号时按「姓名缩写 + 固定后缀 + 工号后6位」拼出来（如 `zs2026138531`）。

**为什么带工号段**：前缀由人工按姓名缩写填，重名的人会填成一样（部门里有两个张磊），
加同一个后缀后密码完全相同 —— 等于两人共用一套账号密码。工号后6位唯一、现成、不用拼音。
**前缀因此变成可选的**，钉钉通讯录导出的 372 行名单可以零手工填写直接导入。

**为什么是后 6 位不是后 4 位**：工号是 8 位但前缀不止一种（`1001xxxx` / `1004xxxx` / `1005xxxx`），
后 4 位实测撞了 5 组（`10033343` 王卢卢 / `10043343` 季星宇 等）。去掉 2 位前缀后的 6 位是流水号，无重合。
...
| `defaultPasswordMode` | `initials+suffix+last6`（当前，带工号后6位）或 `initials+suffix`（老规则）。填别的会直接报错退出 |
| `forceChangeOnFirstLogin` | 当前只做「提示不强制」。置 `true` 也不会拦登录，别指望它当强制手段 |

> 切回 `initials+suffix` 是为了兼容**已发出去的老密码** —— 换公式后那些人按新公式登录不上。
> 前缀为空的名单在老规则下会被判「缺密码前缀」拒掉，脚本会提示。
