...
- `密码前缀`：姓名缩写，**必填**。由人工/Excel 公式生成好带进来（程序不做拼音转换，避免引入依赖和多音字出错）
...
**密码从哪来**：批量建号时按「**姓名缩写（首字母大写） + 年份 + 工号后 4 位**」拼出来。
例：陈丹萍（`cdp`，工号 `1001`**`7968`**）→ `Cdp20267968`。

**重名不用特殊处理**：部门里有两个张磊，缩写都是 `zl`，但工号后 4 位不同 ——
`Zl20263853` / `Zl20263228`，天然区分。

**大小写在公式里统一**：名单里填 `cdp` / `CDP` / `Cdp`，算出来都是 `Cdp2026…`，
免得同一批人因为录入习惯不同拿到两种格式的密码。
...
| `defaultPasswordMode` | `initials+suffix+last4`（当前）或 `initials+suffix`（老规则）。填别的会直接报错退出 |
| `forceChangeOnFirstLogin` | 当前只做「提示不强制」。置 `true` 也不会拦登录，别指望它当强制手段 |

> 切回 `initials+suffix` 是为了兼容**已发出去的老密码**（如 `zs2026`）——
> 换成带工号段的公式后，按老规则拿到密码的人就对不上了。老规则下前缀同样必填。
