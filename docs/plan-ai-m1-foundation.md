...
**身份是假的 → 权限没意义 → 权限没意义 → 审计只是自娱自乐 → 审计不可信 → 不敢让 Agent 自主执行。**
...
| 步骤 | 内容 | 产出 |
|---|---|---|
| **1** | `db.js` + 建表 + 把老审计日志导入 | 数据库跑起来，审计已迁移 |
| **2** | `modules/auth` + 登录页 + session | 能真登录，`/api/auth/me` 可用 |
| **3** | `whoami()` 切换 + 权限矩阵 | 身份真实，权限生效 |
| **4** | 部署脚本 + 备份 + 说明文档 | 可发布 |

**第 1、2 步可以立刻做，不影响任何现有功能**（都是新增，不改旧的）。
**第 3 步是真正有风险的**——它动到所有模块的写路径，需要先备份数据、在测试环境验一遍。
