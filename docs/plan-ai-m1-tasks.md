...
### 步骤 6 · 脱敏规范文档（**不阻塞**，可最后做）【✅ 已完成 2026-09-21】

这是文档活，不阻塞 M1 代码验收。内容：L1/L2/L3 分级 + 字段级脱敏规则 + AI 边界。
主文档 4.5 节已有三级框架，本步把它升级为一份可发布的独立规范，落到本仓库真实字段。

【完成记录】已产出 `docs/plan-ai-m1-data-masking.md`（v1）：
- 红线（L3 不进云端 AI / 工时绝对值不流出 / 姓名假名化不可反查 / 凭据零外泄 / 从严默认）
- L1/L2/L3 分级 → 字段级脱敏对照表（对照 `updatedBy`/`workload`/`iterations[].name`/
  `audit_log.user_name`/`users.password_hash`/`dingtalk_id`/TB `sid` 等真实字段）
- 四种手段（假名化/聚合化/百分比化/统计特征化）→ 场景矩阵（展示/备份/跨系统/喂 AI/回写/日志）
- AI 特化（输入聚合化 + 输出二次校验 + `ai_triggered`/`ai_adopted` 溯源）
- 假名映射表管理（L3 加密 + 反查留痕）→ M2 接入检查清单 → 变更管理
- 已在 `plan-ai-master.md` 4.5 节与 M1 任务表引用并标 ✅

**至此 M1 全部 6 项（系统库/登录/权限/部署/审计/脱敏）代码与文档均完成，仅剩人工验收。**
...
| `plan-ai-m1-data-masking.md` | **数据脱敏规范（v1）**：L1/L2/L3 分级 + 字段级规则 + AI 边界 + M2 接入卡点。M2 接数据/AI 前的红线 |
...
6. 做**步骤 6**（脱敏规范文档）【✅ 已完成：`plan-ai-m1-data-masking.md`】
7. 按第 2 节修正 `plan-ai-master.md` 和 `plan-ai-m1-foundation.md`
8. 更新 `handoff.md`，把这轮新踩的坑追加进去

**M1 做完的标志**：`plan-ai-m1-foundation.md` 第 226-231 行那 6 条验收全部打勾，
且线上以 `AUTH_REQUIRED=1` 跑起来之后，多账号能正常登录取用。
**代码与文档层面已全部完成（含脱敏规范），当前唯一剩余 = 人工验收。**
